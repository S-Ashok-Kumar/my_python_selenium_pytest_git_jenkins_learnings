"""
Command to execute: python -m pytest PlaywrightWithPython/test_pw_tabs_13.py -s -v

"""

from playwright.sync_api import sync_playwright, Page, expect, Playwright


def test_browsercontext(playwright:Playwright):

    browser = playwright.chromium.launch(headless=False)  # Created browser
    context = browser.new_context()     # Created context
    parentpage = context.new_page()   # Created page

    parentpage.goto("https://demo.automationtesting.in/Windows.html")

    #register an event for handle tab
    context.on("page", lambda page: page.wait_for_load_state())

    parentpage.locator("a[href='http://www.selenium.dev'] button").click()

    parentpage.wait_for_timeout(2000)  # allow tab to open
    all_pages = context.pages
    print("Number of tabs/pages: ", len(all_pages))

    print("Title of parent page: ", all_pages[0].title())
    print("Title of child page/tab: ", all_pages[1].title())

    childpage = all_pages[1]
    print("URL of child page",childpage.url)
    parentpage.wait_for_timeout(5000)

    context.close()
    browser.close()

